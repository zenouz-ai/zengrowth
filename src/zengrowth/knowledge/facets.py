...
one batch) or per job through ``InstrumentedLLM`` so spend is
...
class InstrumentedFacetAssigner:
    """Facet assigner backed by the central instrumented LLM client."""
    ...
    def assign(
        ...
        *,
        ...
        return self._llm.chat_json(
            ...
            # Older models receive temperature 0. Newer models that reject
            # custom sampling use their provider default; the closed vocabulary
            # and wholesale row replacement keep the derived counts bounded.
            temperature=0.0,
        )
