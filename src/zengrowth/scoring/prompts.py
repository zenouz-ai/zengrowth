...
Single-agent scoring. Dual-agent critique is AG-04 / Phase 5, not built.
Returns strict JSON; malformed or truncated output goes through the bounded
repair re-ask in ``InstrumentedLLM.chat_json`` (EA-03) and fails loud after that.
"""
...
def build_user_prompt(settings: Settings, *, job: dict, posting_age_days: int | None) -> str:
    profile = {
        ...
        # Mirrors the ingestion recency window instead of a second hard-coded value.
        "recency_preference_days": settings.max_posting_age_days,
    }
    schema = {
