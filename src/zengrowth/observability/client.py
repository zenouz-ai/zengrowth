...
def _allows_custom_sampling(model: str) -> bool:
    """Return whether Anthropic accepts non-default sampling parameters."""
    ...
    def _record_call(
        ...
        *,
        ...
        cost = cost_usd(
            request_model,
            usage,
            price_overrides=self._settings.llm_price_overrides,
    ...
    def chat(
        ...
        *,
        ...
        with span(operation_name, entity_type=entity_type, entity_id=entity_id) as span_id:
            ...
            try:
                # Newer adaptive-thinking models reject non-default sampling
                # parameters. Older models still receive an explicitly requested
                # temperature; otherwise the provider default is used.
                ...
                if temperature is not None and _allows_custom_sampling(model):
        ...
        with span(operation_name, entity_type=entity_type, entity_id=entity_id) as span_id:
            ...
            try:
                ...
                self._record_call(
                    ...
                    entity_id=entity_id,
                    span_id=span_id,
                    web_search_requests=int(web_searches or 0),
                    detail={
