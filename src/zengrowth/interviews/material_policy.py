...
Derived from an operator Director of AI journey comparison (generated vs imported).
...
PREP_PACK_TYPES = ("interviewer_pack", "tech_prep_pack", "final_round_pack", "session_prep")

# Materials that must stay process-/thread-grounded (no Anthropic web_search).
# Everything else may use web when INTERVIEW_RESEARCH_WEB_SEARCH is enabled.
_WEB_SEARCH_DENIED: frozenset[str] = frozenset(
    {
        "session_prep",  # runnable prompt — keep deterministic
        "email_draft",
        "offer_response",
        "answer",  # evidence-grounded application answers
    }
)


def material_allows_web(material_type: str) -> bool:
    """Whether this material type may call Anthropic server-side web_search."""
    return material_type not in _WEB_SEARCH_DENIED
...
CULTURE_COMPARISON_SECTIONS: list[str] = [
    "Goal",
...
SESSION_PREP_SECTIONS: list[str] = [
    "Session Goal",
...
def pack_sections(pack_type: str, *, round_type: str | None = None) -> list[str]:
    ...
    if pack_type == "culture_comparison":
        return list(CULTURE_COMPARISON_SECTIONS)
    if pack_type == "session_prep":
        return list(SESSION_PREP_SECTIONS)
...
def quality_warnings(
    ...
    *,
    ...
    if pt not in {"company_briefing", "culture_comparison", "session_prep"}:
...
3. Web search for net-new company/interviewer facts, current tools, and widely accepted practices
...
- When using web search: prefer primary docs, vendor docs, and widely accepted practices;
  do not invent candidate employers, dates, metrics, or qualifications from the web.
