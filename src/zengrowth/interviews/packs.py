...
def _pack_prompt(
    ...
    *,
    ...
    adapt_snippets: list[dict[str, Any]] | None = None,
) -> str:
    ...
    payload: dict[str, Any] = {
        ...
        "adapt_from_prior_materials": adapt_snippets or None,
        "today": datetime.now(UTC).date().isoformat(),
    }
    if enhance_skeleton:
        payload["enhance_skeleton"] = enhance_skeleton[:12000]
    if pack_type in {"culture_comparison", "session_prep"}:
        intro = (
            f"Prepare a {_PACK_TITLES[pack_type]} for the process described below.\n"
...
def generate_pack(
    ...
    *,
    ...
    adapt_material_ids: list[int] | None = None,
) -> GeneratedMaterial:
    ...
    with pipeline_run(
        ...
        detail={
            ...
            "enhance": enhance,
            "adapt_material_ids": adapt_material_ids or [],
        },
    ):
        ...
        adapt_snippets: list[dict[str, Any]] = []
        for mid in adapt_material_ids or []:
            prior = session.get(GeneratedMaterial, mid)
            if prior is None or prior.job_id == job.id:
                continue
            raw = _read_material_text(prior)
            if not raw:
                continue
            adapt_snippets.append(
                {
                    "material_id": prior.id,
                    "job_id": prior.job_id,
                    "material_type": prior.material_type,
                    "title": prior.title,
                    "body": strip_llm_envelope(raw)[:8000],
                }
            )
        ...
        if adapt_snippets:
            system += (
                "\n\nYou may adapt reusable framing from adapt_from_prior_materials "
                "(other job processes). Re-ground every claim for THIS job; do not "
                "copy employer-specific facts that do not apply."
            )
