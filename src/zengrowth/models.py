"""SQLModel tables for the career OS.

Canonical entities cover the shipped loop: jobs, interviews, offers, generated
materials, the evidence/knowledge graph, and audit/telemetry.
VISION §9 schemas that remain unbuilt (EmailThread, AgentDecision, BudgetRecord,
Config, a separate Company table) are listed in docs/SPEC.md.
"""
...
class Application(SQLModel, table=True):
    """Leftover Phase 1 table. Lifecycle lives on ``Job``; materials live on
    ``GeneratedMaterial``. Rows are never inserted; the table is only cleared
    when a job is deleted. Kept so existing SQLite files keep loading.
    """
...
class OperatorSession(SQLModel, table=True):
    """Server-side session allowlist (SEC-03).

    Login inserts a row keyed by the cookie ``sid``. Logout sets ``revoked_at``.
    The auth gate accepts a cookie only when HMAC, expiry, *and* this row are live,
    so a stolen cookie dies when the operator signs out.
    """
