...
def _pairs(before: list[Any], after: list[Any]) -> list[tuple[Any | None, Any | None]]:
    """Align two line lists, keeping added/removed lines instead of dropping them.

    ``zip`` silently ignores the tail of the longer list, so a line that a later
    pass *deleted* appeared in neither ``lines_total`` nor ``changes`` — the
    change summary would report "no change" for content that is no longer in the
    CV. Padding with ``None`` makes a deletion (or addition) a visible change.
    """
    return list(zip_longest(before, after, fillvalue=None))
...
def summarize_cv_changes(original: dict[str, Any], tailored: dict[str, Any]) -> dict[str, Any]:
    ...
    for index, (before, after) in enumerate(_pairs(orig_caps, new_caps)):
        lines_total += 1
        if normalize_cv_line(before or "") != normalize_cv_line(after or ""):
            lines_changed += 1
            changes.append(
                {
                    "section": "capability",
                    ...
                    "before": before,
                    "after": after,
                    "kind": "removed" if after is None else "added" if before is None else "edited",
                }
            )

    orig_roles = original.get("experience") or []
    new_roles = _tailored_experience(tailored)
    for role_index, (before_role, after_role) in enumerate(_pairs(orig_roles, new_roles)):
        for bullet_index, (before, after) in enumerate(_pairs(before_role or [], after_role or [])):
            lines_total += 1
            if normalize_cv_line(str(before or "")) != normalize_cv_line(str(after or "")):
                lines_changed += 1
                changes.append(
                    {
                        "section": "experience",
