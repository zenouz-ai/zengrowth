...
from ..interviews.service import INTERNAL_MATERIAL_TYPES
...
_MARKDOWN_REVISABLE = frozenset(INTERNAL_MATERIAL_TYPES) | {"answer"}
...
def _compile_tex_material(
    ...
    *,
    ...
    if material_type == "cv" and fit_client is not None:
        ...
        draft_json = dict(draft_json or {})
        if final_tex != tex_content:
            # Legacy CVs without a stored template baseline diff against the
            # pre-fit document, so the change summary shows what the fit changed.
            baseline = draft_json.get("template_baseline") or _parse_cv_template(tex_content)
            draft_json = refresh_cv_draft_after_fit(draft_json, final_tex, baseline)
        # Always record the report — including when every rewrite was rejected and
        # the .tex is unchanged — so the operator can see applied vs rejected rounds.
        draft_json.setdefault("tailoring", {})["page_fit"] = fit_report
    else:
...
def revise_material(
    ...
    *,
    ...
    if source.material_type == "answer" or source.material_type in _MARKDOWN_REVISABLE:
        if payload.mode == "latex":
            raise ValueError("latex edit is not supported for markdown materials")
        if not payload.markdown_body:
            raise ValueError("markdown_body is required for markdown revisions")
        if source.material_type == "answer":
            ...
            _write_metadata(
                out_dir.parent / "metadata.json", material, "not_applicable", settings.material_model()
            )
            return material

        # Internal markdown packs / debriefs / reflections / summaries.
        from ..interviews.markdown_format import slugify
        from ..models import MaterialAudience

        stamp = datetime.now(UTC).strftime("%Y%m%d-%H%M%S-%f")
        out_dir = _material_dir(job) / f"{stamp}-{source.material_type.replace('_', '-')}"
        out_dir.mkdir(parents=True, exist_ok=True)
        md_path = out_dir / f"{slugify(source.title)}.md"
        md_path.write_text(payload.markdown_body, encoding="utf-8")
        version = _next_version(session, job.id or 0, source.material_type)
        material = GeneratedMaterial(
            job_id=job.id or 0,
            interview_id=source.interview_id,
            material_type=source.material_type,
            audience=source.audience or MaterialAudience.internal,
            title=source.title,
            markdown_path=str(md_path),
            evidence_ids=list(source.evidence_ids or []),
            version=version,
            supersedes_id=source.id,
            status="created_markdown" if source.status != "imported" else "imported",
            draft_json={"body": payload.markdown_body},
        ...
        log_action(
            ...
            action=f"revise_{source.material_type}",
            entity_type="job",
            entity_id=job.id,
            detail={"material_id": material.id, "supersedes_id": source.id},
        )
        return material
...
def revise_material_with_instruction(
    ...
    *,
    ...
    if source.material_type == "answer" or source.material_type in INTERNAL_MATERIAL_TYPES:
