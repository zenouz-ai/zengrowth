...
# Word-boundary matched with an optional plural so the denylist catches
# "Campaigns" without swallowing unrelated titles: plain substring matching
# archived "Head of AI, International Markets" ("intern"), "Director of ML,
# Internal Platforms" ("intern"), and "Salesforce AI Lead" ("sales").
_HARD_EXCLUDE_RE = re.compile(
    r"\b(?:" + "|".join(re.escape(t) for t in _HARD_EXCLUDE_TERMS) + r")s?\b",
    re.IGNORECASE,
)


def _excluded(title: str) -> bool:
    return _HARD_EXCLUDE_RE.search(title) is not None
