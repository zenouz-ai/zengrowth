...
from .ats_greenhouse import _html_to_text as html_to_text
...
def parse_lever_job(slug: str, payload: dict) -> Job:
    ...
    # ``descriptionPlain`` is already plain text; the ``description`` fallback is
    # HTML and must be cleaned the same way Greenhouse content is.
    description = (payload.get("descriptionPlain") or "").strip() or html_to_text(
