...
class MaterialPackRequest(BaseModel):
    """Generate an internal prep pack (INT-02 / INT-11)."""
...
class MaterialMetaPatch(BaseModel):
    """Relink or reclassify an imported/generated internal material (INT-08)."""
...
class PromoteLearningRequest(BaseModel):
    """Queue a debrief/journey insight as a draft evidence claim (INT-04/INT-10)."""

    claim_text: str = Field(min_length=1)
    category: str = "interview_learning"
...
class RejectionReflectionRequest(BaseModel):
    """Optional override for pasted rejection feedback (INT-07)."""

    notes: str | None = None


class JourneySummaryEligibility(BaseModel):
    eligible: bool
    proper_round_count: int
