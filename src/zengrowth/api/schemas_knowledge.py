...
class CloseoutLearningCandidate(BaseModel):
    """A portable skill or next-interview rule extracted from close-out docs."""
...
class CloseoutLearningsOut(BaseModel):
    candidates: list[CloseoutLearningCandidate]


class CloseoutPromoteRequest(BaseModel):
    """Operator-selected close-out bullets to queue as draft claims."""

    claim_texts: list[str] = Field(min_length=1)


class CloseoutPromoteResponse(BaseModel):
    """Draft claims pulled from operator-selected close-out bullets."""
