...
def create_answer(
    ...
    """Queue an application answer; return immediately so edge proxies do not 502.

    Long LLM generation runs in a background task. Poll ``GET .../materials/{id}``
    until ``status`` is no longer ``generating`` (ready: ``created_markdown``,
    failed: ``failed`` with error in ``draft_json.error``).
    """
    job = session.get(Job, job_id)
    if not job:
        raise HTTPException(status_code=404, detail="job not found")
    ...
    return _to_material_out(pending)
...
def _run_answer_generation(
    ...
    """Complete a queued answer on a fresh session (request session is already closed)."""
    try:
        with Session(get_engine()) as session:
            ...
            if not material or not job:
                _logger.error(
                    "answer generation aborted: material=%s job=%s missing",
                    material_id,
                    job_id,
                )
                return
            try:
                ...
                # Grounding / validation failures are operator-actionable, not 502s.
                mark_answer_failed(session, material, str(exc))
                _logger.info("answer generation failed for material %s: %s", material_id, exc)
            except RuntimeError as exc:
                mark_answer_failed(session, material, str(exc))
                _logger.warning("answer generation runtime error for material %s: %s", material_id, exc)
            except Exception as exc:  # pragma: no cover - must not crash the worker
                mark_answer_failed(session, material, f"Generation failed: {exc}")
                _logger.exception("answer generation crashed for material %s", material_id)
    except Exception:  # pragma: no cover
        _logger.exception("answer generation session failed for material %s", material_id)
