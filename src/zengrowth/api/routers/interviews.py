...
from ..schemas import (
    ...
    JourneySummaryEligibility,
    MaterialImportRequest,
    MaterialMetaPatch,
    MaterialPackRequest,
    PromoteLearningRequest,
    RejectionReflectionRequest,
    SimPromptRequest,
)
from ..schemas_knowledge import (
    CloseoutLearningCandidate,
    CloseoutLearningsOut,
    CloseoutPromoteRequest,
    CloseoutPromoteResponse,
    EvidenceClaimOut,
)
...
@router.post(
    "/jobs/{job_id}/promote-learning",
    response_model=EvidenceClaimOut,
    status_code=201,
)
def promote_job_learning_endpoint(
    job_id: int,
    payload: PromoteLearningRequest,
    session: Session = Depends(get_session),
) -> EvidenceClaimOut:
    """Promote a journey/rejection insight without tying it to one round (INT-10)."""
    job = _get_job(session, job_id)
    try:
        claim, _created = promote_learning(
            session,
...
@router.get("/jobs/{job_id}/closeout-learnings", response_model=CloseoutLearningsOut)
def list_closeout_learnings_endpoint(
    job_id: int,
    session: Session = Depends(get_session),
) -> CloseoutLearningsOut:
    """Portable skills and next-interview rules from close-out documents."""
    job = _get_job(session, job_id)
    candidates = list_closeout_learning_candidates(session, job)
    return CloseoutLearningsOut(
        candidates=[
            CloseoutLearningCandidate(
                category=category, claim_text=text, source_title=source
            )
            for category, text, source in candidates
        ]
    )


@router.post(
    "/jobs/{job_id}/promote-closeout-learnings",
    response_model=CloseoutPromoteResponse,
    status_code=201,
)
def promote_closeout_learnings_endpoint(
    job_id: int,
    payload: CloseoutPromoteRequest,
    session: Session = Depends(get_session),
) -> CloseoutPromoteResponse:
    """Queue selected close-out bullets as draft claims (INT-07/10). Never auto-verified."""
    job = _get_job(session, job_id)
    try:
        claims, promoted, skipped = promote_closeout_learnings(
            session, job, claim_texts=payload.claim_texts
        )
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    return CloseoutPromoteResponse(
        promoted=promoted,
        skipped_existing=skipped,
        claims=[EvidenceClaimOut.model_validate(claim) for claim in claims],
    )
...
def create_pack(
    ...
    try:
        material = generate_pack(
            ...
            adapt_material_ids=payload.adapt_material_ids,
        )
    except Exception as exc:
        raise llm_http_exception(exc) from exc
    from .jobs import _to_material_out
...
@router.patch("/jobs/{job_id}/materials/{material_id}/meta", response_model=GeneratedMaterialOut)
def patch_material_meta(
    job_id: int,
    material_id: int,
    payload: MaterialMetaPatch,
    session: Session = Depends(get_session),
) -> GeneratedMaterialOut:
    job = _get_job(session, job_id)
    ...
    try:
        updated = update_material_meta(
            session,
            job,
            material,
            interview_id=payload.interview_id,
            clear_interview=payload.clear_interview,
            title=payload.title,
            material_type=payload.material_type,
        )
    except ValueError as exc:
    ...
    return _to_material_out(updated)


@router.delete("/jobs/{job_id}/materials/{material_id}", status_code=204)
def remove_material(
    job_id: int,
    material_id: int,
    session: Session = Depends(get_session),
    ...
    if not material or material.job_id != job_id:
        raise HTTPException(status_code=404, detail="material not found")
    try:
        delete_material(session, job, material)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    return Response(status_code=204)


@router.get("/jobs/{job_id}/materials/adaptable")
def adaptable_materials(
    job_id: int,
    material_type: str | None = None,
    session: Session = Depends(get_session),
) -> list[dict]:
    """List recent internal packs from other jobs for cross-job adapt (INT-11)."""
    _get_job(session, job_id)
    rows = list_adaptable_materials(
        session, exclude_job_id=job_id, material_type=material_type
    )
    return [
        {
            "id": material.id,
        ...
        for material, job in rows
    ]


@router.post(
    "/jobs/{job_id}/materials/rejection-reflection",
    response_model=GeneratedMaterialOut,
    status_code=201,
)
def create_rejection_reflection(
    job_id: int,
    payload: RejectionReflectionRequest | None = None,
    session: Session = Depends(get_session),
) -> GeneratedMaterialOut:
    job = _get_job(session, job_id)
    notes = payload.notes if payload else None
    if notes is not None and notes.strip() and notes.strip() != (job.outcome_notes or ""):
        job.outcome_notes = notes.strip()
        session.add(job)
        session.commit()
        session.refresh(job)
    try:
        material = generate_rejection_reflection(session, job, feedback_notes=notes)
    except Exception as exc:
        raise llm_http_exception(exc) from exc
    from .jobs import _to_material_out
...
@router.get("/jobs/{job_id}/journey-summary/eligibility", response_model=JourneySummaryEligibility)
def journey_eligibility(
    job_id: int, session: Session = Depends(get_session)
) -> JourneySummaryEligibility:
    job = _get_job(session, job_id)
    from ...interviews.journey import TERMINAL_OUTCOMES

    return JourneySummaryEligibility(
        eligible=journey_summary_eligible(session, job),
        proper_round_count=count_proper_interviews(session, job.id or 0),
        terminal_outcome=job.outcome_result in TERMINAL_OUTCOMES,
    )


@router.post(
    "/jobs/{job_id}/materials/journey-summary",
    response_model=GeneratedMaterialOut,
    status_code=201,
)
def create_journey_summary(
    ...
    job = _get_job(session, job_id)
    try:
        material = generate_journey_summary(session, job)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except Exception as exc:
        raise llm_http_exception(exc) from exc
