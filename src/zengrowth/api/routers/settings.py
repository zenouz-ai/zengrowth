...
class OperatorProfile(BaseModel):
    """Non-secret operator profile used to prefill materials (OFF-06)."""
...
def get_profile() -> OperatorProfile:
    ...
    return OperatorProfile(
        ...
        current_company=settings.user_current_company,
        current_role=settings.user_current_role,
        manager_name=settings.user_manager_name,
        notice_period=settings.user_notice_period,
        location=settings.user_location,
        scoring_model=settings.scoring_model,
        generation_model=settings.material_model(),
    )
