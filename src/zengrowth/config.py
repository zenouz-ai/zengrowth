...
class Settings(BaseSettings):
    ...
    # Default chat model for scoring and (unless overridden) materials generation.
    # Anthropic current generation: claude-sonnet-5 (balance), claude-opus-5 (max quality).
    scoring_model: str = "claude-sonnet-5"
    # Optional override for CV/pack/answer generation; falls back to scoring_model.
    ...
    # Current employer profile — prefills the departure pack (OFF-06). Empty by
    # default so source stays synthetic; operators set real values in .env.
    user_current_company: str = ""
    user_current_role: str = ""
    user_manager_name: str = ""
    user_notice_period: str = ""
    ...
    # The ceiling meters recorded ``LlmCall`` rows, so it only works while
    # ``feature_observability`` is on; turning telemetry off also turns the cap
    # off. The check is pre-call, so the call that crosses the line still runs.
    ...
    # Which direct peers may set the client address the throttle keys on
    # (``X-Real-IP`` / first ``X-Forwarded-For``). Loopback only by default:
    # trusting *any* private address would trust every sibling container on a
    # shared Docker network, so a compromised neighbour could forge a fresh
    # client IP per attempt and walk past the lockout entirely. Prefer
    # ``login_proxy_token`` whenever nginx shares a Docker network with the API.
    # If you must use peer trust instead, pin the exact observed peer as a /32
    # (never a shared subnet such as 172.18.0.0/16) — see RUNBOOK §8a. Entries
    # are IPs or CIDR blocks; anything else is keyed on the peer address itself.
    login_trusted_proxies: list[str] = Field(default_factory=lambda: ["127.0.0.1", "::1"])
    # Preferred over ``login_trusted_proxies`` wherever the API shares a Docker
    # network with other containers: peer addresses cannot distinguish nginx from
    # a sibling, but a shared secret can. When set, the forwarded client address is
    # trusted only on requests carrying this value in ``X-ZenGrowth-Proxy-Token``,
    # which the edge adds and a sibling container cannot guess.
    login_proxy_token: str | None = None
    ...
    # Superseded material versions are only deleted when this is on. It defaults
    # to off because the purge removes rows *and* files from disk: an operator
    # opts in rather than discovering deletions after an upgrade. When off, run
    # ``python -m zengrowth.materials.retention purge`` manually.
    materials_retention_purge_enabled: bool = False
    materials_retention_purge_hour: int = 4
    ...
    def material_model(self) -> str:
        """Model id used for materials generation (packs, answers, offers, etc.)."""
        return self.generation_model or self.scoring_model
