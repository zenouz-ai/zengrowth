...
```
ATS pull / paste-to-fill / bookmarklet
        │
        ▼
   Dedup + precheck ──► (obvious non-targets archived, no LLM cost)
        │
        ▼
   Clean + score (Claude, strict JSON, model-compatible sampling)
        │
        ▼
...
- `middleware.py`, `security.py`, `operator_sessions.py`, `login_throttle.py`, `routers/auth.py` —
  protected-by-default routes, PBKDF2-SHA256 password hashing, HMAC-signed
  cookies with a server-side session allowlist (logout revokes the session),
  and login backoff.
...
- `facets.py`, `coverage.py` — coverage-map faceting against a closed,
  ...
  strict-JSON call per document/job, key-guarded and never fatal to
