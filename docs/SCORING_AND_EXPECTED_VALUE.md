...
explainable **priority score**. The full rationale is stored on the row — scores
are never hidden behind a single number. Claude Sonnet 5 rejects non-default
sampling parameters, so ZenGrowth omits `temperature` for that model instead of
sending an invalid request; live provider outputs are not guaranteed identical.
...
A deterministic regression suite runs in CI against *frozen* scorer outputs — it
pins the aggregation and the weights, and does not exercise the live model:
rewording the same JD may move the priority score by at most
