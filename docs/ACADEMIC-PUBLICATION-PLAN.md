...
**Status:** living research plan, 2026-08-26

**Current artifact:** working paper v0.4 (working system and prospective evaluation)
...
| Steerable scoring | [Attune](https://arxiv.org/abs/2608.14948) makes pairwise-derived scoring criteria inspectable and editable and evaluates the approach technically and with domain experts. | Explainable scores are not novel by themselves. Measure stability, operator disagreement, and correction burden; do not imply ZenGrowth has Attune's interactive steering. |
| Resume generation | [ResumeFlow](https://arxiv.org/abs/2402.06221) already covers personalized resume generation and hallucination-oriented metrics. [Grounded Optimization](https://arxiv.org/abs/2607.01457) already ablates layered rewrite defenses on synthetic resumes. | Broader lifecycle coverage, an enforced outbound-action boundary, and a live field account with correction burden — still unmeasured. |
...
| Employer-side field evidence | [Better Together](https://arxiv.org/abs/2507.08029) reports two recruitment-platform field experiments and an applicant completion cost. [No Thoughts Just AI](https://arxiv.org/abs/2509.04404) shows that biased simulated AI recommendations can alter human resume-screening decisions. | Adjacent evidence, not proof that candidate-side tailoring works. Treat human approval as a mechanism to evaluate, not a guarantee, and report costs shifted to applicants. |
...
|---|---|---|
| G0 | v0.4 working-system/prospective-evaluation paper | Capability boundaries, claims, references, and public metadata pass accuracy review. |
