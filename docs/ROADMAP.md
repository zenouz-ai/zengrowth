...
- **Explainable scoring** — single strict-JSON Claude call returning per-dimension
  ...
  (frozen calibration fixtures protect the ranking logic; repeated-run live-model
  stability remains an evaluation target).
...
- **Interview workflow** — dated, backdatable rounds; three-tier prep/debrief
  ...
  Intact + iwoca operator replay is done; fixture quality gates ship in CI.
...
- **Interview pack quality** — deterministic INT-06 fixture tests and enhance
  mode are shipped; operator replay of Intact + iwoca is done. Optional
