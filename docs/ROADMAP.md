...
- **Explainable scoring** — single strict-JSON Claude call returning per-dimension
  ...
  (frozen regression fixtures pin the ranking logic and weights; repeated-run
  live-model stability remains an evaluation target).
- **Paste-to-fill** — manually add a job by pasting its description, or send
  one with the Add-this-JD bookmarklet (URL stored only; the page is never
  fetched).
...
- **Security & ops** — operator auth, fail-closed boot, encrypted keyring, login
  backoff, session revocation on logout, k-anonymized public surface, ingestion
  heartbeat + `/health/ready` probe, and a default-off daily LLM-spend ceiling.
...
- **Interview workflow** — dated, backdatable rounds; three-tier prep/debrief
  ...
  debriefs; email drafts; promote-a-learning; close-out learning review;
  deterministic simulator prompt.
  Operator journey replay is done; synthetic fixture quality gates ship in CI.
  Judged pack-quality eval remains planned.
...
- **Interview pack quality** — deterministic INT-06 fixture tests and enhance
  mode are shipped; operator journey replay is done. Optional
  LLM-as-judge eval remains.
- **Retrieval** — evidence is selected by JD relevance over verified claims.
  Chunk embeddings are **off by default** (`KNOWLEDGE_EMBEDDINGS_ENABLED`) and are
  not read by any retrieval path. Flip on only when a semantic/hybrid retriever
