...
No value needs editing for the first local run. Leave `ANTHROPIC_API_KEY` blank
to connect it in the dashboard setup wizard, or set it in `.env` before boot.
In dev, leave `ZENGROWTH_REQUIRE_HTTPS=false`; the auth gate then falls back to
a localhost bypass so you can run without configuring an operator.
...
3. **Add your first job** on *Add Job*: paste a job description (or install the
   Add-this-JD bookmarklet from that page) and submit. The posting URL is stored
   as a reference only and is never fetched. Manual entry creates the row
   immediately; "Prepare application" then cleans and scores it (this is the
   first step that spends LLM tokens).
