...
import { getOperatorProfile, getSettingsStatus, saveApiKey, uploadKnowledgeSource } from '../lib/api'
...
export function Setup() {
  ...
  const profile = useAsyncData(() => getOperatorProfile(), [])
  ...
  return (
    <div className="mx-auto mt-16 max-w-xl px-6">
      ...
      <div className="mt-6">
        <Panel title="Operator profile (departure pack)">
          <div className="flex flex-col gap-2 text-sm text-muted">
            <p>
              Prefills the leave-well departure form. Edit these in <code>.env</code> (
              <code>USER_CURRENT_COMPANY</code>, <code>USER_CURRENT_ROLE</code>,{' '}
              <code>USER_MANAGER_NAME</code>, <code>USER_NOTICE_PERIOD</code>) and restart the API.
            </p>
            {p ? (
              <dl className="grid grid-cols-2 gap-2 text-xs">
                <div>
                  <dt className="text-muted">Current company</dt>
                  <dd className="text-text">{p.current_company || '—'}</dd>
                </div>
                <div>
                  <dt className="text-muted">Current role</dt>
                  <dd className="text-text">{p.current_role || '—'}</dd>
                </div>
                <div>
                  <dt className="text-muted">Manager</dt>
                  <dd className="text-text">{p.manager_name || '—'}</dd>
                </div>
                <div>
                  <dt className="text-muted">Notice period</dt>
                  <dd className="text-text">{p.notice_period || '—'}</dd>
                </div>
                <div className="col-span-2">
                  <dt className="text-muted">Models</dt>
                  <dd className="text-text">
                    scoring {p.scoring_model} · materials {p.generation_model}
                  </dd>
                </div>
              </dl>
            ) : (
              <p className="text-xs">Profile values load from the API after Setup connects.</p>
            )}
          </div>
        </Panel>
      </div>
      ...
      <p className="mt-2 text-center text-xs text-muted">
        After Claude is connected,{' '}
        <Link to="/add" className="text-cyan underline">
          Add this JD
        </Link>{' '}
        on Add job is a bookmarklet for filing a posting in one tap.
      </p>
    </div>
  )
}
