...
export async function generateAnswer(
  ...
  const { data } = await api.post<GeneratedMaterial>(
    ...
    // Queue returns 202 immediately; generation finishes in the background.
    { timeout: 30_000 },
  )
  return data
}

/** Poll until an async answer leaves ``generating`` (or timeout). */
export async function waitForAnswer(
  ...
  for (;;) {
    const detail = await getMaterial(jobId, materialId)
    if (detail.status !== 'generating') {
      if (detail.status === 'failed') {
        const err =
          (detail.draft_json as { error?: string } | null)?.error ||
    ...
    if (Date.now() - started > timeoutMs) {
      throw new Error('Answer generation timed out — refresh and check the answer list.')
    }
...
export async function generatePack(
  ...
  const { data } = await api.post<GeneratedMaterial>(
    `/jobs/${jobId}/materials/pack`,
    {
      ...
      adapt_material_ids: opts?.adaptMaterialIds?.length ? opts.adaptMaterialIds : null,
    },
    { timeout: LONG_TIMEOUT },
  )
  return data
...
export async function patchMaterialMeta(
  jobId: number,
  materialId: number,
  payload: {
    interview_id?: number | null
    clear_interview?: boolean
    title?: string | null
    material_type?: string | null
  },
): Promise<GeneratedMaterial> {
  const { data } = await api.patch<GeneratedMaterial>(
    `/jobs/${jobId}/materials/${materialId}/meta`,
    payload,
  )
  return data
}

export async function listAdaptableMaterials(
  jobId: number,
  materialType?: string,
): Promise<
  Array<{
...
> {
  const { data } = await api.get(`/jobs/${jobId}/materials/adaptable`, {
    params: materialType ? { material_type: materialType } : undefined,
  })
  return data
}

export async function reviseMaterialMarkdown(
  jobId: number,
  materialId: number,
  markdownBody: string,
): Promise<GeneratedMaterial> {
  const { data } = await api.patch<GeneratedMaterial>(`/jobs/${jobId}/materials/${materialId}`, {
    mode: 'structured',
...
export async function generateRejectionReflection(
  jobId: number,
  notes?: string | null,
): Promise<GeneratedMaterial> {
  const { data } = await api.post<GeneratedMaterial>(
    `/jobs/${jobId}/materials/rejection-reflection`,
    { notes: notes ?? null },
    { timeout: LONG_TIMEOUT },
  )
  return data
...
export async function getJourneySummaryEligibility(jobId: number): Promise<{
  eligible: boolean
  proper_round_count: number
  terminal_outcome: boolean
}> {
  const { data } = await api.get(`/jobs/${jobId}/journey-summary/eligibility`)
  return data
}

export async function generateJourneySummary(jobId: number): Promise<GeneratedMaterial> {
  const { data } = await api.post<GeneratedMaterial>(
    `/jobs/${jobId}/materials/journey-summary`,
    null,
    { timeout: LONG_TIMEOUT },
  )
  return data
...
export async function promoteLearning(
  ...
  interviewId: number,
  claimText: string,
  category: 'interview_learning' | 'process_skill' = 'interview_learning',
): Promise<EvidenceClaim> {
  const { data } = await api.post<EvidenceClaim>(
    `/jobs/${jobId}/interviews/${interviewId}/promote-learning`,
    { claim_text: claimText, category },
  )
  invalidateKnowledgeClaimsCache()
  return data
}

export async function promoteJobLearning(
  jobId: number,
  claimText: string,
  category: 'interview_learning' | 'process_skill' = 'interview_learning',
): Promise<EvidenceClaim> {
  const { data } = await api.post<EvidenceClaim>(`/jobs/${jobId}/promote-learning`, {
...
export async function listCloseoutLearnings(jobId: number): Promise<CloseoutLearnings> {
  const { data } = await api.get<CloseoutLearnings>(`/jobs/${jobId}/closeout-learnings`)
  return data
}

export async function promoteCloseoutLearnings(
  jobId: number,
  claimTexts: string[],
): Promise<CloseoutPromoteResult> {
  const { data } = await api.post<CloseoutPromoteResult>(
    `/jobs/${jobId}/promote-closeout-learnings`,
...
export async function getOperatorProfile(): Promise<OperatorProfile> {
  const { data } = await api.get<OperatorProfile>('/settings/profile')
  return data
}
