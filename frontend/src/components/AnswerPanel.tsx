...
import { apiErrorMessage, downloadMaterial, generateAnswer, getMaterial, waitForAnswer } from '../lib/api'
...
export function AnswerPanel({ jobId, answers, onUpdated }: AnswerPanelProps) {
  ...
  async function onGenerate() {
    ...
    setError(undefined)
    try {
      const pending = await generateAnswer(jobId, question, limit ? Number(limit) : undefined)
      onUpdated()
      await waitForAnswer(jobId, pending.id)
      setQuestion('')
      setLimit('')
      onUpdated()
    } catch (err) {
      setError(apiErrorMessage(err, err instanceof Error ? err.message : 'Generate answer failed. Try again.'))
      onUpdated()
    } finally {
      setBusy(false)
    }
