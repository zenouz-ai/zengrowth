...
function DepartureForm({
  ...
  useEffect(() => {
    ...
    void getOperatorProfile()
      .then((profile) => {
        if (cancelled) return
        if (profile.current_company) setCurrentCompany(profile.current_company)
        if (profile.current_role) setCurrentRole(profile.current_role)
        if (profile.manager_name) setManagerName(profile.manager_name)
        ...
        if (
          ...
          setPrefillNote('Prefilled from your operator profile (Setup / .env). Edit before generating.')
        }
      })
      .catch(() => {
        /* profile is optional — form stays blank */
      })
    return () => {
      cancelled = true
    }
  ...
  return (
    <div className="mt-3 flex flex-col gap-2 rounded-lg border border-border/70 bg-black/20 p-3">
      ...
      {prefillNote && <p className="text-xs text-cyan">{prefillNote}</p>}
