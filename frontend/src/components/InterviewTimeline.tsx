...
import {
  ...
  getInterview,
  importMaterial,
  listAdaptableMaterials,
  promoteLearning,
  setInterviewTranscript,
  updateInterview,
} from '../lib/api'
import { materialTypeLabel } from '../lib/materialLabels'
import { suggestInternalMaterialType } from '../lib/suggestMaterialType'
import {
...
const PACK_BUTTON_LABELS: Record<PackType, string> = {
  ...
  culture_comparison: 'Culture comparison',
  session_prep: 'Session / Claude Code prep',
}
...
export function InterviewTimeline({
  ...
  const [adaptIds, setAdaptIds] = useState<number[]>([])
  const adaptable = useAsyncData(() => listAdaptableMaterials(jobId), [jobId])
  ...
  async function runPack(
    ...
    opts?: { enhance?: boolean; sourceMaterialId?: number; adaptMaterialIds?: number[] },
  ) {
    ...
    try {
      await guard(
        () =>
          generatePack(jobId, packType, interviewId, {
            ...opts,
            adaptMaterialIds: adaptIds.length ? adaptIds : undefined,
          }),
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button
          type="button"
          disabled={generating !== null}
          onClick={() => runPack('culture_comparison', null)}
        ...
        <button
          type="button"
          disabled={generating !== null}
          onClick={() => runPack('session_prep', null)}
          className="rounded-lg border border-violet px-3 py-2 text-sm text-violet disabled:opacity-50"
        >
          Session / Claude Code prep
        </button>
      ...
      {(adaptable.data?.length ?? 0) > 0 && (
        <details className="rounded-lg border border-border/70 bg-black/10 p-3 text-sm">
          <summary className="cursor-pointer text-xs text-muted">
            Adapt from prior materials ({adaptIds.length} selected)
          </summary>
          <ul className="mt-2 flex max-h-40 flex-col gap-1 overflow-y-auto text-xs">
            {adaptable.data!.map((row) => {
              const checked = adaptIds.includes(row.id)
              return (
                <li key={row.id}>
                  <label className="flex cursor-pointer items-start gap-2">
                    <input
                      ...
                      onChange={() =>
                        setAdaptIds((ids) =>
                          checked ? ids.filter((id) => id !== row.id) : [...ids, row.id],
                        )
                      }
                    />
                    <span>
                      <span className="text-text">{row.company}</span> — {row.title}{' '}
                      <span className="text-muted">({row.material_type.replace(/_/g, ' ')})</span>
                    </span>
                  </label>
                </li>
              )
...
function ImportArtifactForm({
  ...
  const [busy, setBusy] = useState(false)
  const [error, setError] = useState<string>()
  const suggestedType = suggestInternalMaterialType(title, content)
  const showTypeHint = suggestedType != null && suggestedType !== materialType
  ...
  return (
    <div className="mt-3 flex flex-col gap-3 rounded-lg border border-border/70 bg-white/[0.02] p-4">
      <p className="text-xs text-muted">
        ...
        Claude Code / practice briefs → Session prep. Culture notes → Culture comparison.
        Rejection write-ups → Rejection reflection.
        {interviewId != null ? (
          <>
            {' '}
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <label className="flex flex-col gap-1 text-xs text-muted">
          ...
          {showTypeHint && suggestedType && (
            <button
              type="button"
              onClick={() => setMaterialType(suggestedType)}
              className="self-start text-left text-xs text-cyan hover:underline"
            >
              Looks like {materialTypeLabel(suggestedType)} — use this type
            </button>
          )}
        </label>
        <label className="flex flex-col gap-1 text-xs text-muted sm:col-span-2">
          ...
          <input
            value={title}
            onChange={(e) => setTitle(e.target.value)}
            placeholder="e.g. Northwind research pack"
            className={inputClass}
          />
        </label>
      </div>
