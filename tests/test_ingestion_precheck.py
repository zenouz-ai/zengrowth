...
@pytest.mark.parametrize(
    "title",
    [
        "Head of AI, International Markets",
        "Director of Machine Learning, Internal Platforms",
        "Salesforce AI Lead",
    ],
)
def test_hard_exclude_is_word_boundary_matched(title: str) -> None:
    """Substring matching archived senior AI roles on "intern"/"sales" (TA-02)."""
    signal = relevance_signal(_job(title))
    assert signal.relevant, f"{title} was excluded by reason={signal.reason}"


@pytest.mark.parametrize(
    "title",
    ["Sales Engineer, AI", "AI Campaigns Manager", "Data Science Intern", "AI Internship Programme"],
)
def test_hard_exclude_still_archives_non_target_titles(title: str) -> None:
    signal = relevance_signal(_job(title))
    assert not signal.relevant
    assert signal.reason == "hard_excluded"
