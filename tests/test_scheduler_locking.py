...
def test_retention_purge_is_scheduled_only_when_enabled(monkeypatch):
    """The 30-day purge is documented as automatic; it must actually be scheduled."""
    from zengrowth import scheduler as sched_mod
    from zengrowth.config import get_settings
    ...
    monkeypatch.setattr(sched_mod, "BackgroundScheduler", lambda daemon=True: FakeScheduler())
    monkeypatch.setenv("INGESTION_CATCHUP_ON_START", "false")

    for enabled, expected in ((False, False), (True, True)):
        monkeypatch.setenv("MATERIALS_RETENTION_PURGE_ENABLED", str(enabled).lower())
        get_settings.cache_clear()
        sched_mod._scheduler = None
        added.clear()
        sched_mod.start_scheduler()
        sched_mod.stop_scheduler()
        assert ("materials_retention_purge" in added) is expected
