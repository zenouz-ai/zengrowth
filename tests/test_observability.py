...
def test_chat_forwards_temperature_only_when_set(session, monkeypatch):
    ...
    llm.chat_json(
        system="s", user="u", model="claude-sonnet-5", max_tokens=10,
        operation_name="op", session=session, temperature=0.0,
    )
    assert "temperature" not in captured
...
def test_cost_usd_charges_cached_input_separately():
    """Anthropic reports input_tokens exclusive of cache counts (they are additive)."""
    usage = TokenUsage(
        input_tokens=1000, output_tokens=500, cache_read_tokens=2000, cache_creation_tokens=1000
    )
    # 1000*3 + 500*15 + 2000*0.30 + 1000*3.75 per 1M
    expected = (1000 * 3.0 + 500 * 15.0 + 2000 * 0.30 + 1000 * 3.75) / 1_000_000
    assert cost_usd("claude-sonnet-5", usage) == pytest.approx(expected, rel=1e-9)


def test_cost_usd_includes_server_side_web_search_requests():
    usage = TokenUsage(input_tokens=1000, output_tokens=500)
    tokens_only = cost_usd("claude-sonnet-5", usage)
    with_search = cost_usd("claude-sonnet-5", usage, web_search_requests=5)
    assert with_search == pytest.approx(tokens_only + 5 * 0.01, rel=1e-9)
    override = cost_usd(
        "claude-sonnet-5", usage, price_overrides={"web_search": {"per_request": 0.02}},
        web_search_requests=2,
    )
    assert override == pytest.approx(tokens_only + 0.04, rel=1e-9)
