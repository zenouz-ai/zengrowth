...
class FakePackClient:
    ...
    def generate_document(self, system, user, model, max_tokens, *, operation_name, allow_web=True):  # noqa: ANN001
        ...
        return self.markdown, self.citations, self.web and allow_web
...
def test_generate_company_briefing(session: Session, tmp_path: Path, monkeypatch):
    ...
    client = FakePackClient(
        _pack_markdown(),
        citations=[{"url": "https://example.com/northwind", "title": "Northwind overview"}],
    )
    ...
    assert "# Company briefing — Northwind" in document
    ...
    assert "https://example.com/northwind" in document
    assert material.markdown_path.endswith("company-briefing-northwind.md")
