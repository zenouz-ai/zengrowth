...
def _job(session: Session, company: str = "Northwind") -> Job:
    job = Job(
        company=company,
...
def test_interview_404s(session: Session, client: TestClient):
    job = _job(session)
    other = _job(session, company="Fabrikam")
...
def test_import_material_backdated(session: Session, client: TestClient, tmp_path: Path, monkeypatch):
    ...
    ).read_text(encoding="utf-8").startswith("# Northwind")
...
def test_import_material_detail_and_download(
    ...
    assert detail["fallback_content"].startswith("# Northwind")
...
def test_import_reclassifies_culture_notes_from_briefing_default(
    session: Session, client: TestClient, tmp_path: Path, monkeypatch
):
    monkeypatch.setattr(generator, "MATERIALS_ROOT", tmp_path / "materials")
    job = _job(session)
    resp = client.post(
        ...
        json=_import_payload(
            title="Culture comparison vs current employer",
            content=(
    ...
    assert resp.status_code == 201
    assert resp.json()["material_type"] == "culture_comparison"


def test_import_reclassifies_claude_code_brief_from_tech_pack(
    session: Session, client: TestClient, tmp_path: Path, monkeypatch
):
    monkeypatch.setattr(generator, "MATERIALS_ROOT", tmp_path / "materials")
    job = _job(session)
    resp = client.post(
        ...
        json=_import_payload(
            ...
            title="Claude Code practice brief",
            content="## Session Goal\nTimed exercise.\n\n## Runnable Prompt\nReview this.\n",
        ),
    )
    assert resp.status_code == 201
    assert resp.json()["material_type"] == "session_prep"
