...
def test_cross_job_learnings_only_verified_and_other_jobs(session: Session):
    northwind = _job(session, "Northwind")
    fabrikam = _job(session, "Fabrikam")
    draft_claim, _ = promote_learning(session, fabrikam, claim_text="Draft learning stays out.")
    verified_claim, _ = promote_learning(session, fabrikam, claim_text="Lead with platform ROI story.")
    verified_claim.verification_state = ClaimVerificationState.verified
    session.add(verified_claim)
    own_claim, _ = promote_learning(session, northwind, claim_text="Northwind-specific learning.")
    own_claim.verification_state = ClaimVerificationState.verified
    session.add(own_claim)
    session.commit()

    learnings = load_cross_job_learnings(session, exclude_job_id=northwind.id)
    assert "Lead with platform ROI story." in learnings
    assert "Draft learning stays out." not in learnings
    assert "Northwind-specific learning." not in learnings
