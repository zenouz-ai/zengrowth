...
def test_cover_letter_revise_and_download(session: Session, tmp_path, monkeypatch, client_with_session: TestClient):
    ...
    revised = client_with_session.patch(
        f"/api/jobs/{job.id}/materials/{material.id}",
        json={"mode": "structured", "draft": {"body": "Dear Globex,\n\nUpdated hook."}},
    )
...
def test_fit_pages_shortens_long_cv(session: Session, tmp_path, monkeypatch, client_with_session: TestClient):
    ...
    class FitClient:
        def complete_text(self, system, user, model, max_tokens=8000, **kwargs):
            state["pages"] = 2
            # A genuine trim: same wording, tighter layout (the fit gate rejects
            # any rewrite that adds content absent from the CV or evidence).
            return r"\documentclass{article}\begin{document}\vspace{-2pt}CV\end{document}"
...
def test_fit_pages_records_rejected_report_when_tex_unchanged(
    session: Session, tmp_path, monkeypatch, client_with_session: TestClient
):
    """A rejected rewrite must still leave tailoring.page_fit for the operator."""
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr(generator, "MATERIALS_ROOT", tmp_path / "materials")
    # Detail enrichment reads the CV template for a change_summary; stub it so
    # the chdir'd tmp workspace does not depend on the repo checkout.
    monkeypatch.setattr(
        "zengrowth.materials.generator._read_cv_template",
        lambda: r"\documentclass{article}\begin{document}CV\end{document}",
    )
    ...
    monkeypatch.setattr("zengrowth.materials.revise.compile_pdf", fake_compile)
    monkeypatch.setattr("zengrowth.materials.generator.compile_pdf", fake_compile)
    monkeypatch.setattr("zengrowth.materials.generator.measure_pdf_extent", lambda pdf: (3, 0.5))

    class RejectClient:
        def complete_text(self, system, user, model, max_tokens=8000, **kwargs):
            # Introduce a non-typography command so the fit gate rejects and the
            # on-disk .tex stays identical to the pre-fit source.
            return r"\documentclass{article}\begin{document}\phantom{hidden}CV\end{document}"

    monkeypatch.setattr(
        "zengrowth.materials.revise._build_client",
        lambda settings, session=None, entity_id=None: RejectClient(),
    )
    job = _job(session)
    material = _cv_material(session, job, tmp_path / "materials")
    material.page_count = 3
    material.draft_json = None
    session.add(material)
    session.commit()
    ...
    assert response.status_code == 200
    data = response.json()
    revised = session.get(GeneratedMaterial, data["id"])
    assert revised is not None
    page_fit = ((revised.draft_json or {}).get("tailoring") or {}).get("page_fit")
    assert page_fit is not None
    assert page_fit.get("rejected")
    assert not page_fit.get("applied")
    # Also exposed on the detail payload (not only in the DB row).
    assert (data.get("tailoring") or {}).get("page_fit", {}).get("rejected")
