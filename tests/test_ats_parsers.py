...
def test_parse_greenhouse_decodes_entity_encoded_content():
    """Greenhouse returns content as HTML-entity-encoded HTML (`content=true`)."""
    payload = {
        "title": "Head of AI",
        "content": "&lt;p&gt;Lead the AI team.&lt;/p&gt;&lt;p&gt;R&amp;amp;D focus.&lt;/p&gt;",
    }
    job = parse_greenhouse_job("acme", payload)
    assert job.description == "Lead the AI team. R&D focus."
    assert "<p>" not in (job.description or "")
    assert "&lt;" not in (job.description or "")


def test_parse_greenhouse_prefers_first_published_over_updated_at():
    """Recency filtering must use the posting date, not the last-edit date."""
    payload = {
        "title": "Head of AI",
        "first_published": "2026-01-02T00:00:00Z",
        "updated_at": "2026-09-01T00:00:00Z",
    }
    assert parse_greenhouse_job("acme", payload).posting_date == date(2026, 1, 2)
    # Falls back to updated_at when the board omits first_published.
    assert parse_greenhouse_job(
        "acme", {"title": "X", "updated_at": "2026-09-01T00:00:00Z"}
    ).posting_date == date(2026, 9, 1)


def test_parse_lever_cleans_html_description_fallback():
    payload = {"text": "Lead AI", "description": "<p>Own the <b>roadmap</b>.</p>"}
    assert parse_lever_job("acme", payload).description == "Own the roadmap."
