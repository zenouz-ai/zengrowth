...
def test_jobs_extract_endpoint_returns_structured_fields(monkeypatch):
    def fake_extract_job_fields(*, raw_text: str, application_url: str | None = None):
        ...
        return ExtractedJobFields(
            ...
            compensation={"min_gbp": 90000, "max_gbp": 110000},
            seniority="Director",
    ...
    assert data["compensation"] == {"min_gbp": 90000, "max_gbp": 110000}
...
def test_answer_endpoint_passes_word_limit(monkeypatch, session: Session):
    ...
    def fake_generate_answer(
        ...
        **_kwargs,
        ...
        if pending_material is not None:
            pending_material.title = "Answer"
            pending_material.status = "created_markdown"
            pending_material.draft_json = {"body": "Because.", "title": "Answer"}
    ...
    monkeypatch.setattr("zengrowth.api.routers.jobs._run_answer_generation", run_on_test_session)
    client = TestClient(app)
    try:
        created = client.post("/api/jobs", json={"company": "Acme", "title": "AI Lead", "source": "manual"}).json()
        response = client.post(
        ...
        assert response.status_code == 202
        body = response.json()
        assert body["material_type"] == "answer"
        assert body["status"] in {"generating", "created_markdown"}
        assert captured == {"question": "Why us?", "word_limit": 200, "instructions": "Concise"}
        refreshed = session.get(GeneratedMaterial, body["id"])
        assert refreshed is not None
        assert refreshed.status == "created_markdown"
    finally:
        app.dependency_overrides.clear()


def test_answer_endpoint_rejects_empty_question(session: Session):
    def override_get_session() -> Iterator[Session]:
        yield session
