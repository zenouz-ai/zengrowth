...
def _job() -> Job:
    return Job(
        ...
        source="manual",
        dedup_hash=dedup_hash("Northwind", "Director of AI", None),
    )


def test_build_frontmatter_matches_obsidian_shape():
    fm = build_frontmatter("Company briefing — Northwind", _job(), material_type="company_briefing")
    assert fm.startswith("---\n")
    assert 'title: "Company briefing — Northwind"' in fm
...
def test_wrap_obsidian_pack_envelope():
    ...
    doc = wrap_obsidian_pack(
        body,
        title="Company briefing — Northwind",
        job=_job(),
    ...
    assert "# Company briefing — Northwind\n" in doc
...
def test_fixture_snippet_shape():
    snippet = Path(__file__).parent / "fixtures" / "replay-research-pack-snippet.md"
    text = snippet.read_text(encoding="utf-8")
    assert text.startswith("---\n")
    assert "# Northwind Research" in text
    assert "> [!tip]" in text
