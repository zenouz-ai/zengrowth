...
def test_alias_key_folds_case_punctuation_and_legal_suffixes() -> None:
    ...
    assert alias_key("Aldermont AG") == "aldermont"
...
def test_deduplicate_knowledge_reports_alias_merges(session: Session) -> None:
    session.add_all(
        [
            KnowledgeEntity(name="Aldermont", normalized_name="aldermont", entity_type="employer"),
            KnowledgeEntity(
                name="Aldermont AG", normalized_name="aldermont ag", entity_type="employer"
            ),
        ]
    )
    session.commit()
    ...
    assert [e.name for e in session.exec(select(KnowledgeEntity)).all()] == ["Aldermont"]
