...
<p align="center">
  ...
  It discovers and scores senior AI and Data-Science roles, grounds application materials in verified evidence claims,
  and carries the same auditable context through interviews, offers, onboarding, and departure.
</p>
...
> **Disclaimer.** This is a personal, non-commercial research and development
> project maintained in a personal capacity. It is unrelated to and
> independent of my professional employment, generates no commercial revenue,
> and is not affiliated with any employer or registered business. Shared for
> research and demonstration purposes only.

## Why you'll like it

- ⚡ **Keep the whole journey together.** Discover and score roles, generate and review materials, prepare for interviews, evaluate offers, and turn an accepted role into a 30/60/90-day plan. Per-call spend is visible on the dashboard; end-to-end time, review burden, and per-application cost remain under formal evaluation.
...
- 🧾 **Transparent and review-first.** Scores retain rationales; generation uses only verified evidence claims (auto-verified above a confidence bar when the cited span checks out, otherwise operator-reviewed); numeric/entity gates catch selected fabrication classes; logged workflow events support audit. Semantic fidelity — whether a claim means what its source span says, and whether a sentence is entailed by its evidence — still requires human review.

## The problem

Senior career moves are high-effort and fragmented. Roles are scattered across ATS feeds, descriptions are noisy, and the evidence gathered during applications rarely carries cleanly into interviews, offer decisions, or onboarding. ZenGrowth supports **discover → score → tailor → review → interview → decide → start** while keeping every outbound action under human control.
...
1. **Discover** — roles arrive from Greenhouse/Lever ATS pulls, paste-to-fill, or the Add-this-JD bookmarklet (URL stored only, never fetched).
...
Meaningful workflow transitions are written to a live audit log on the dashboard. Log completeness has not been audited yet: treat the log as evidence for reconstruction, not as proof that every action was recorded.
...
- **Explainable scoring.** One strict-JSON Claude call returns per-dimension scores, a stored rationale, and an observable-fit priority score. Frozen regression fixtures pin the ranking logic; the live model's score stability is measured, not assumed, and provider outputs are not claimed to be identical across runs. → [`docs/SCORING_AND_EXPECTED_VALUE.md`](docs/SCORING_AND_EXPECTED_VALUE.md)
- **Evidence-constrained materials.** Structure-preserving CV tailoring, cover letters, and answers draw from verified evidence claims; deterministic gates detect selected unsupported numeric/entity tokens (new figures, CamelCase/acronym names — single-capitalised names are not detected), and page-fit rewrites pass the same gates, while operator review remains responsible for sentence-level support. → [`docs/MATERIAL_GENERATION.md`](docs/MATERIAL_GENERATION.md)
...
```bash
cp .env.example .env          # local defaults; API keys can stay blank
...
| Models | Anthropic Claude (default `claude-sonnet-5`; optional `GENERATION_MODEL` for packs); OpenAI embeddings optional, off by default |
...
The current [working paper](output/pdf/zengrowth-grounded-not-automated-v0.4.pdf),
