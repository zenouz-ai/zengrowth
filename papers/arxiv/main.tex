...
operated since May 2026 for a live senior AI/DS search. Its pipeline:
...
only when the extractor's confidence clears a configured bar, the cited span
exists in the source, and the claim's high-risk tokens match that span, while
the rest queue for operator review. Generation receives only verified claims,
or, when the bank holds none, a hand-authored evidence file the operator
maintains. Write-time gates reject numbers and a conservative subset of
named entities found in neither the evidence bank nor the job context. These
allowlists catch important fabrication classes --- invented figures, and
uncited names in CamelCase or acronym form --- but they are token allowlists,
not entailment checks. Single-capitalised names (most employer names) are not
detected, an allowed token may be reused in an unsupported context, and
auto-verification itself checks hard facts against the cited span rather than
meaning. Whether each sentence is entailed by its evidence, and how often the
auto-verified pool is a faithful paraphrase of its source, are both subjects of
the planned claim-level audit. Alias resolution merges entity surface variants. A deterministic,
...
scoring regression suite (rewording stability bounds, hand-ranked monotonicity,
anchor drift alarms) --- all CI-enforced with zero LLM spend. These run the
gate and aggregation logic over small hand-authored fixture sets, each with a
negative case proving the check is not vacuous; they are regression protection
for the mechanisms, not measurements of generated output or of live scorer
stability, which remain the prospective quantities in
Table~\ref{tab:pending}.
...
CV page-fit rewrites face the same discipline plus structure-preserving
checks (no new ungrounded figures/entities/words, no change to bullet or
section counts, and no added content-hiding constructs) before the fitted
\texttt{.tex} is kept.
