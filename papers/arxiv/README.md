...
only. Copy a reviewed PDF to `output/pdf/` when freezing a public render (only
the current frozen render is tracked; superseded ones stay local).

**Source ahead of the frozen render (2026-09).** `main.tex` has since been
corrected for accuracy — deployment start date, the precise scope of the
entity gate, and the status of the CI eval gates (regression tests over
hand-authored fixtures, not measurements). The tracked v0.4 PDF predates those
corrections; rebuild before circulating it.
